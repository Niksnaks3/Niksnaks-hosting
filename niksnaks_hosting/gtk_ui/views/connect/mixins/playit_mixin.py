...
class PlayitMixin:
    ...
    def _exact_compatible_modrinth_version(self, project_id: str):
        ...
        for version in versions:
            loaders = [str(l).lower() for l in (version.loaders or [])]
    ...
    def _install_tunnel_mod(self, project_id: str, title: str) -> tuple[bool, str]:
        ...
        if not version:
            loader_name = get_loader_display_name(self._server_loader())
            return False, _("{} is not available for {} on Minecraft {}").format(
                title, loader_name, self._server_info.mc_version
            )
    ...
    def _confirm_required_mod_install(self, tunnel_name: str, mods: list[tuple[str, str]], on_confirm):
        ...
        loader_name = get_loader_display_name(self._server_loader())
        dialog.set_body(
            _("This tunnel needs {}. Niksnaks-Hosting can install compatible {} versions automatically.").format(names, loader_name)
        )
